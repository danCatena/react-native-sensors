...
public class Orientation extends ReactContextBaseJavaModule implements SensorEventListener {
  ...
  public void isAvailable(Promise promise) {
    if (this.sensor == null) {
      ...
      promise.reject(new RuntimeException("No Orientation found"));
      return;
    }
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
      ...
      if (tempMs - lastReading >= interval){
        ...
        if (mySensor.getType() == Sensor.TYPE_ORIENTATION) {
          map.putDouble("z", sensorEvent.values[0]);
          map.putDouble("x", sensorEvent.values[1]);
          map.putDouble("y", sensorEvent.values[2]);
